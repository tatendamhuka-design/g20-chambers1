...
export default function BarristerCard({ barrister, onQuickView }) {
  return (
    <div className="bg-white rounded-xl p-6 border border-[#e8e0d4] hover:shadow-lg transition-all hover:-translate-y-1 group h-full flex flex-col">
      {/* Avatar / Image - Now shows image if available */}
      <Link href={`/barristers/${barrister.slug}`} className="flex-1 flex flex-col">
        <div className="w-24 h-24 mx-auto rounded-full bg-[#0a1628] flex items-center justify-center text-2xl font-bold text-white mb-4 group-hover:bg-[#c9a84c] transition-colors overflow-hidden">
      ...
      {/* Actions */}
      <div className="mt-4 flex gap-2 justify-center border-t border-[#e8e0d4] pt-4">
        <Link
